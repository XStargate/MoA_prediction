...
from decompo import decompo_process_kfold


def train_tabnet_kfold(x_train, y_train, x_test, submission, feature_cols, target_cols, seeds, nfolds, save_path):
    ...
    for seed in seeds:
        ...
        for fold in range(nfolds):
            ...
            train_df, valid_df, x_test_kfold, pca_features = decompo_process_kfold(train_df, valid_df, x_test,
                                                                                   decompo="PCA",
                                                                                   genes_variance=0.85,
                                                                                   cells_variance=0.9,
                                                                                   seed=42,
            ...
            feature_cols_kfold = list(feature_cols)+list(pca_features)
            print(len(feature_cols_kfold))
            print(f"train_df shape: {train_df.shape}")
            print(f"valid_df shape: {valid_df.shape}")
            print(f"x_test_kfold shape: {x_test_kfold.shape}")

            x_tr, y_tr = train_df[feature_cols_kfold].values, train_df[target_cols].values
            x_val, y_val = valid_df[feature_cols_kfold].values, valid_df[target_cols].values
            x_train_kfold = pd.concat([train_df, valid_df], ignore_index=True)
            # print(x_train_kfold[feature_cols_kfold].shape)
            ...
            # Save OOF for CV
            preds_tr = model.predict(x_train_kfold[feature_cols_kfold].values)
            ...
            # Predict on test
            preds_test = model.predict(x_test_kfold[feature_cols_kfold].values)
...
def pred_tabnet_kfold(x_train, y_train, x_test, submission, feature_cols, target_cols, seeds, nfolds, load_path, stacking=False):
    cfg_fe = Config_FeatureEngineer()
    seed_everything(seed_value=cfg_fe.seed)
    ...
    for seed in seeds:
        ...
        for fold in range(nfolds):
            ...
            x_tot, y_tot = x_train_kfold[feature_cols_kfold].values, y_train[target_cols].values
