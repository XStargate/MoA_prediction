...
def _pca(train_features, test_features, runty, save_path, load_path, ncomp_g=463, ncomp_c=60):
    ...
    data_ = pd.concat([pd.DataFrame(train_features[GENES]), pd.DataFrame(test_features[GENES]),
                      pd.DataFrame(test_features[GENES]), pd.DataFrame(test_features[GENES]),
                      pd.DataFrame(test_features[GENES])])
    data = pd.concat([pd.DataFrame(train_features[GENES]), pd.DataFrame(test_features[GENES])])
    if (runty == 'traineval'):
        gpca = (pca_g.fit(data_[GENES]))
    ...
    data_ = pd.concat([pd.DataFrame(train_features[CELLS]), pd.DataFrame(test_features[CELLS]), 
                       pd.DataFrame(test_features[CELLS]), pd.DataFrame(test_features[CELLS]),
                       pd.DataFrame(test_features[CELLS])])
    data = pd.concat([pd.DataFrame(train_features[CELLS]), pd.DataFrame(test_features[CELLS])])
    if (runty == 'traineval'):
        cpca = (pca_c.fit(data_[CELLS]))
...
def process(train_features, test_features, train_targets_scored, 
            train_targets_nonscored, train_drug, runty, save_path, load_path):
